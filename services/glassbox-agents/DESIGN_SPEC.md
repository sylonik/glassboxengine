...
and are invoked by the TS API behind `GLASSBOX_AGENT_SERVICE_URL`.

A single **Coordinator** root agent routes a JSON `task` to one of three structured-output
sub-agents (mirrors the GlassBox agent names):
...
## Tools Required
None external. Agents are pure LLM reasoning over data supplied in the request; the
database, embeddings, and ranking all live in the TS side. This keeps the agents
stateless and safe to run on Agent Engine.
